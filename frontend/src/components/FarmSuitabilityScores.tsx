import { Gauge, Layers, CloudSun, Droplets, Activity } from 'lucide-react';
import type { Language, SuitabilityScores } from '../types/agriculture';
import { t } from '../utils/translations';

interface FarmSuitabilityScoresProps {
    suitability: SuitabilityScores;
    lang: Language;
}

export function FarmSuitabilityScores({ suitability, lang }: FarmSuitabilityScoresProps) {
    const categories = [
        {
            key: 'soil',
            label: 'Soil Suitability',
            score: suitability.soil,
            icon: Layers,
            color: 'emerald',
            desc: 'Soil pH & physical matrix suitability'
        },
        {
            key: 'weather',
            label: 'Weather Suitability',
            score: suitability.weather,
            icon: CloudSun,
            color: 'sky',
            desc: 'Temperature & relative atmospheric humidity'
        },
        {
            key: 'water',
            label: 'Water / Rainfall Fit',
            score: suitability.water,
            icon: Droplets,
            color: 'blue',
            desc: 'Seasonal rainfall & crop water requirement'
        },
        {
            key: 'nutrients',
            label: 'Nutrient Balance (NPK)',
            score: suitability.nutrients,
            icon: Activity,
            color: 'teal',
            desc: 'Nitrogen, Phosphorus, Potassium levels'
        }
    ];

    const getBadgeStyle = (score: number) => {
        if (score >= 75) return { text: t('good', lang), cls: 'bg-[#00B884]/20 text-[#00B884] border-[#00B884]/30' };
        if (score >= 55) return { text: t('moderate', lang), cls: 'bg-amber-500/20 text-amber-300 border-amber-500/30' };
        return { text: t('needsAttention', lang), cls: 'bg-rose-500/20 text-rose-300 border-rose-500/30' };
    };

    return (
        <div className="bg-[#102D25] rounded-3xl p-6 sm:p-7 shadow-xl border border-white/10 mb-8 text-[#F3EBDD]">
            
            {/* HEADER */}
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-6 pb-4 border-b border-white/10">
                <div className="flex items-center gap-3">
                    <div className="p-2.5 rounded-xl bg-[#00B884]/15 text-[#00B884] font-bold border border-[#00B884]/30">
                        <Gauge className="w-5 h-5 text-[#00B884]" />
                    </div>
                    <div>
                        <h2 className="text-lg font-black text-[#F3EBDD]">
                            {t('farmSuitability', lang)}
                        </h2>
                        <p className="text-xs text-[#A8B9AE] font-medium">
                            Multi-dimensional agronomic compatibility scoring
                        </p>
                    </div>
                </div>

                {/* OVERALL BADGE */}
                <div className="flex items-center gap-2 bg-[#0C241E] border border-white/10 px-4 py-2 rounded-2xl">
                    <span className="text-xs text-[#A8B9AE] font-bold">Overall Score:</span>
                    <span className="text-lg font-black text-[#00B884]">{suitability.overall}%</span>
                    <span className={`text-[10px] font-black px-2.5 py-0.5 rounded-full border ${
                        suitability.overall >= 75 ? 'bg-[#00B884]/20 text-[#00B884] border-[#00B884]/30' :
                        suitability.overall >= 55 ? 'bg-amber-500/20 text-amber-300 border-amber-500/30' :
                        'bg-rose-500/20 text-rose-300 border-rose-500/30'
                    }`}>
                        {getBadgeStyle(suitability.overall).text}
                    </span>
                </div>
            </div>

            {/* SUITABILITY PROGRESS BARS GRID */}
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {categories.map((cat) => {
                    const badge = getBadgeStyle(cat.score);
                    const IconComponent = cat.icon;

                    return (
                        <div key={cat.key} className="p-4 rounded-xl border border-white/5 bg-[#0C241E] hover:bg-[#143B30]/60 transition">
                            <div className="flex items-center justify-between mb-2">
                                <div className="flex items-center gap-2">
                                    <div className="p-1.5 rounded-lg bg-[#00B884]/15 text-[#00B884]">
                                        <IconComponent className="w-4 h-4" />
                                    </div>
                                    <span className="text-xs font-bold text-[#F3EBDD]">{cat.label}</span>
                                </div>
                                <div className="flex items-center gap-2">
                                    <span className="text-sm font-extrabold text-[#F3EBDD]">{cat.score}%</span>
                                    <span className={`text-[10px] font-bold px-2 py-0.5 rounded-md border ${badge.cls}`}>
                                        {badge.text}
                                    </span>
                                </div>
                            </div>

                            {/* PROGRESS BAR */}
                            <div className="w-full bg-[#071C17] h-2.5 rounded-full overflow-hidden mb-1.5 border border-white/5">
                                <div
                                    className={`h-full rounded-full transition-all duration-500 ${
                                        cat.score >= 75 ? 'bg-[#00B884]' : cat.score >= 55 ? 'bg-amber-400' : 'bg-rose-400'
                                    }`}
                                    style={{ width: `${cat.score}%` }}
                                />
                            </div>

                            <p className="text-[11px] text-[#A8B9AE]">{cat.desc}</p>
                        </div>
                    );
                })}
            </div>

        </div>
    );
}
