    ...
    useEffect(() => {
        ...
        const checkModel = async () => {
            try {
                ...
                if (status === "loaded") setModelStatus("AI model ready");
                else if (status === "loading") setModelStatus("AI model is loading...");
                else setModelStatus("AI model needs configuration");
            } catch {
                if (active) setModelStatus("Unable to check AI model");
            }
        };
        checkModel();
        const timer = window.setInterval(checkModel, 5000);
    ...
    return <div className="min-h-screen bg-slate-50 text-slate-900 pb-16">
        ...
        <main className="max-w-5xl mx-auto px-5 py-10">
            <div className="text-center">
                ...
                <div className="mt-3 inline-flex rounded-full bg-emerald-50 px-3 py-1 text-xs font-bold text-emerald-700">{modelStatus}</div>
            </div>
                ...
                <div className="rounded-3xl bg-white border border-slate-200 p-6 shadow-sm">
                    ...
                    {!result ? <div className="mt-16 text-center text-slate-400"><p>Upload an image to view the prediction.</p><p className="mt-2 text-xs">AI output is decision support, not a substitute for expert agronomic diagnosis.</p></div> :
                        <div className="mt-8">
                            ...
                            {result.guidance && <div className="mt-6 rounded-2xl bg-emerald-50 p-4 text-sm">
                                <p className="font-black text-emerald-800">Recommended action</p>
                                <p className="mt-1 text-slate-700">{result.guidance.treatment}</p>
                                <p className="mt-3 font-black text-emerald-800">Prevention</p>
                                <p className="mt-1 text-slate-700">{result.guidance.prevention}</p>
                            </div>}
                            <p className="mt-4 text-xs text-slate-400">AI output is decision support only. Confirm serious or uncertain cases with a local agriculture expert.</p>
                        </div>}
                </div>
            </div>
        </main>
